...
final class Notify[F[_]: Monad](
  ...
  def apply(
    ...
    Message.render(template, params) match
      ...
      case Right(message) =>
        alreadyAccepted(idempotencyKey).flatMap:
          ...
          case Right(None)            => accept(recipient, template, params, preferred, idempotencyKey, message)
  ...
  private def accept(
    ...
    directory.lookup(recipient).flatMap:
      case Left(error) => error.asLeft[NotifyOutcome].pure[F]
      case Right(found) =>
        chosen(found, preferred) match
          case Nil => NotificationError.NoChannelAvailable(recipient).asLeft[NotifyOutcome].pure[F]
          case addresses =>
            for
              id <- ids.next
              createdAt <- time.now
              record = Notification(id, recipient, template, params, idempotencyKey, Nil, createdAt)
  ...
  private def deliver(
    ...
    addresses
      ...
      .map(attempts => NotifyOutcome(id, alreadyAccepted = false, attempts).asRight[NotificationError])
  ...
  private def attempt(
    ...
    senderFor.get(channel) match
      case None =>
        time.now.map(DeliveryAttempt(channel, DeliveryStatus.Unreachable, 0, Some("no sender for this channel"), _))
      ...
      case Some(sender) =>
        for
          ...
          attempt = result match
            ...
            case Left(error) => DeliveryAttempt(channel, DeliveryStatus.Failed, 1, Some(error.detail), at)
